...
"""
Save plots to PNG files instead of displaying them interactively.
Useful when running on systems without GUI or when you want to review plots later.
"""
...
arg_parser.add_argument("--output", type=str, default="plots", help="Output directory for PNG files")
...
print(f"Found {len(logs_obj.logs)} trajectories to plot")
print(f"Saving plots to: {args.output}/")
print()

for log in logs_obj.logs:
    ...
    if has_speed:
        ...
        f, (ax1, ax3) = plt.subplots(2, sharex=True, figsize=(12, 6))
...
print(f"  Total: {len(logs_obj.logs)} plots")
print()
print("To view the plots:")
print(f"  open {args.output}/")
print("  Or use: Preview app, or any image viewer")
